...
def admin_backup():
    ...
    try:
        payload = backup_service.make_backup_zip()
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{backup_service.default_backup_name()}"'
        },
    )
...
async def admin_restore(file: UploadFile = File(...)):
    ...
    try:
        backup_service.restore_db_from_bytes(payload)
    except backup_service.BackupError as e:
        raise HTTPException(400, str(e)) from None
